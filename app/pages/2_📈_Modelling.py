...
def select_dataset(datasets: list[Dataset]) -> pd.DataFrame:
    ...
    data_name = st.selectbox('Select the dataset you want to use', list(dataset_map.keys()), placeholder="Choose a dataset")
...
def plot(data: pd.DataFrame):
    ...
    if len(columns) == 3:
        try:
            ...
            st.error("""One of the variables you selected is a catagorical value. It is not possible to plot this in a 3d plot.
...
if not datasets:
    st.write("You currently don't have any datasets uploaded. Please upload your datasets on the Dataset page.")
else:
